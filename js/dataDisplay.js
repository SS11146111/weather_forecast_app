    ...
    if(type === "current")
    ...
    else if(type === "forecast")
        {
            ...
            .then(json => {
                ...
                storedValuesf.push(json.current.uv,json.current.cloud,json.current.dewpoint_c,json.current.feelslike_c,json.current.heatindex_c,json.current.precip_in,json.current.pressure_in,json.current.wind_mph,json.current.wind_dir,json.current.wind_degree,json.current.gust_kph);
                storedKeysf.push("UV index","Cloud","Dew point (c)","Temperature feels like (c)","Heat index(c)","Precipitation amount(in)","Pressure(in)","Wind(m/h)","Wind direction","wind degree","Wind gust(k/h)");
   
                let table1 = document.getElementById("table1");
                for(let i=0; i<storedKeysf.length; i++)
                {
                    let row = document.createElement("tr");
                    let col1 = document.createElement("td");
                    let col2 = document.createElement("td");
                    col1.innerHTML = storedKeysf[i];
                    col1.style.padding = "2px";
                    col1.style.fontSize = "small"
                    col2.innerHTML = storedValuesf[i];
                    col2.style.padding = "2px";
                    col2.style.fontSize = "small"
                    row.appendChild(col1);
                    row.appendChild(col2);
                ...
                storedAirKeysf.push("Carbon Monoxide (μg/m3)","Ozone (μg/m3)","Nitrogen dioxide (μg/m3)","Sulphur dioxide (μg/m3)","PM2.5 (μg/m3)","PM10 (μg/m3)");
                storedAirValuesf.push(json.current.air_quality.co,json.current.air_quality.o3,json.current.air_quality.no2,json.current.air_quality.so2,json.current.air_quality.pm2_5,json.current.air_quality.pm10)
       
                    let table2 = document.getElementById("table2");
                    for(let i=0; i<storedAirKeysf.length; i++)
                    {
                        let row = document.createElement("tr");
                        let col1 = document.createElement("td");
                        let col2 = document.createElement("td");
                        col1.innerHTML = storedAirKeysf[i];
                        col1.style.padding = "2px";
                        col1.style.fontSize = "small"
                        col2.innerHTML = storedAirValuesf[i];
                        col2.style.padding = "2px";
                        col2.style.fontSize = "small"
                        row.appendChild(col1);
                        row.appendChild(col2);
                    ...
                    storedLocKeysf.push("Country","Latitude","Longitude","Local time","Local time epoch","City","Region","Time Zone");
                    storedLocValuesf.push(json.location.country,json.location.lat,json.location.lon,json.location.localtime,json.location.localtime_epoch,json.location.name,json.location.region,json.location.tz_id)
       
                    let table4 = document.getElementById("table4");
                    for(let i=0; i<storedLocKeysf.length; i++)
                    {
                        let row = document.createElement("tr");
                        let col1 = document.createElement("td");
                        let col2 = document.createElement("td");
                        col1.innerHTML = storedLocKeysf[i];
                        col1.style.padding = "2px";
                        col1.style.fontSize = "small"
                        col2.innerHTML = storedLocValuesf[i];
                        col2.style.padding = "2px";
                        col2.style.fontSize = "small"
                        row.appendChild(col1);
                        row.appendChild(col2);
